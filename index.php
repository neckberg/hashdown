...
function obj_parse_hd( String $file_path ) {
  ...
  foreach ($a_hd_lines as $i_line => $s_line) {
    $a_status = get_action_for_line($s_line, $a_status, $a_key_cursor_location, $i_list_depth, $o_object, $a_text_value_current);
    // echo PHP_EOL . 'line ' . $i_line . ': ' . $a_status[0];
  }
  set_object_key($o_object, $a_key_cursor_location, implode(PHP_EOL, $a_text_value_current));
  $a_text_value_current = [];
  // echo PHP_EOL;
  // print_r($o_object);
  return $o_object;
...
function get_action_for_line (String $s_line, Array $a_status, &$a_key_cursor_location, &$i_list_depth, &$o_object, &$a_text_value_current) {
  ...
  $a_line_type = is_line_new_key($s_line);
  ...
  if ( $a_line_type[0] === 'array' ) {
    ...
    array_push($a_key_cursor_location, get_object_next_numeric_key($o_object, $a_key_cursor_location));
    if ( $a_line_type[1] ) {
...
function is_line_new_key (String $s_line) {
  $s_line = ltrim($s_line);
  $a_special_chars = [
    '#' => 'object',
    '-' => 'array',
...
function get_object_next_numeric_key(&$array, $keys = []) {
  $current = &$array;
  foreach($keys as $key) {
    $current = &$current[$key];
    if ( ! is_array($current) ) {
...
function set_object_key(&$array, $keys = [], $value = '') {
  $current = &$array;
  foreach($keys as $key) {
    $current = &$current[$key];
  	if ( ! is_array($current) ) {
