...
These are the two biggest usability gaps to address next:

- [x] **Comments in `.md` files** — `<!-- ... -->` full-line, multi-line, and inline (outside fenced literals). See README and `tests/data/comments.md`.
...
- [ ] **Full-document round-trip tests** — Complex fixtures (`page-builder`, `person`, `todo-list`, `auto-typing`) are only tested one-way (parse *or* write). Add write → read → assert-same tests for real-world confidence.
...
- [ ] **Remove dead/stale parser code** — Unused variables in `x_parse_md_lines` (`$is_in_literal`, `$x_data_cursor`, etc.). Commented-out comment-line and `>` blockquote support — implement or remove.
...
- [ ] **Custom parse exceptions** — Parse errors throw generic `\Exception`. A `HashdownParseException` with line number, file path, and offending line would be nicer for callers.
...
- [x] **Comment syntax** — `<!-- -->` (Markdown/HTML). Removed stubbed `\` line-comment code.
...
- Round-trip coverage for scalars, whitespace, and literals
- README documents auto-typing, inline backticks, and serialization
- Lossless scalar serialization design
- Error cases for bad hash/list depth

## Suggested starting shortlist

1. **Comments in files** (user priority)
2. **Better parse exceptions** (user priority)
3. Full-document round-trips (`page-builder`, `auto-typing`)
4. Fix README groceries example + `false on failure` doc mismatch
5. Align PHP version requirements
6. Add CI
7. `undefined` sentinel
